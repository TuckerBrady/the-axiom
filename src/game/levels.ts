...
export const levelA1_1: LevelDefinition = {
  ...
  computationalGoal: 'Route a signal in a straight line from Input Port to Output Port.',
  conceptTaught: 'Data movement. Information travels along a path with a direction.',
  prerequisiteConcept: 'None. First level.',
  tapeDesignRationale: 'No tape. Layer 1 only. The player learns signal routing before encountering what the signal carries.',
  difficultyBand: 'intuitive',
  narrativeFrame: 'The ship has no power. The most basic system needs the most basic fix: connect source to destination.',
  tutorialHints: [
...
export const levelA1_2: LevelDefinition = {
  ...
  computationalGoal: 'Route a signal that requires one direction change to reach the Output Port.',
  conceptTaught: 'Direction change. The Gear is an omnidirectional fitting that bends signal flow.',
  prerequisiteConcept: 'Signal routing with Conveyors (A1-1).',
  tapeDesignRationale: 'No tape. Layer 1 only. Routing can include direction changes before encountering anything that evaluates the signal.',
  difficultyBand: 'intuitive',
  narrativeFrame: 'Life support is not a straight line. The path bends. Keeping the crew alive requires more than a straight pipe.',
  tutorialHints: [
...
export const levelA1_3: LevelDefinition = {
  ...
  computationalGoal: 'Route a signal through a gate that opens only when the player-set condition matches the Data Trail value.',
  conceptTaught: 'Conditional gating. The Config Node reads the Data Trail and gates signal on a match.',
  prerequisiteConcept: 'Direction change with Gear (A1-2). Signal can be routed. Now it can be gated.',
  tapeDesignRationale: 'No tape. The Data Trail is pre-set to all 1s. The Config Node defaults to 0. The player must tap to set configValue to 1 to match the trail.',
  difficultyBand: 'derivable',
  narrativeFrame: 'Navigation requires conditions. The gate does not open automatically. The Engineer decides when conditions are met.',
  tutorialHints: [
...
export const levelA1_4: LevelDefinition = {
  ...
  budget: 20,
  scoringCategoriesVisible: ['efficiency', 'chainIntegrity', 'protocolPrecision'],
  computationalGoal: 'Route a signal that requires two direction changes to reach the Output Port.',
  conceptTaught: 'Multi-bend routing. Some paths require multiple Gears. Planning the full route before placing pieces.',
  prerequisiteConcept: 'Single-bend routing with Gear (A1-2).',
  tapeDesignRationale: 'No tape. Layer 1 only. The player masters physical routing complexity before encountering state or conditions in tape-based levels.',
  difficultyBand: 'derivable',
  narrativeFrame: 'Propulsion requires precision routing through damaged hull. Structural damage blocks the shortcut. The Engineer must plan around what is broken.',
  tutorialHints: [
...
export const levelA1_5: LevelDefinition = {
  ...
  conceptTaught: 'Reading input into memory and gating on that memory value. The Scanner bridges input tape to Data Trail.',
  prerequisiteConcept: 'Conditional gating with Config Node (A1-3). Now the gate condition comes from the tape, not from static trail values.',
  tapeDesignRationale: 'Mixed tape [1,0,1,1,0] with both values. Tests gate open (1s) and gate block (0s). Two 0s at positions 2 and 5 test blocking at different points.',
  difficultyBand: 'derivable',
  narrativeFrame: 'Communication requires reading the incoming signal. The Scanner reads. The gate decides. Not every signal gets through.',
  tutorialHints: [
...
export const levelA1_6: LevelDefinition = {
  ...
  budget: 40,
  scoringCategoriesVisible: ['efficiency', 'chainIntegrity', 'protocolPrecision'],
  computationalGoal: 'Multiple gates all reading from the same dynamic Scanner write. Every gate must receive the correct live value.',
  conceptTaught: 'Dynamic state. The Data Trail is live memory that changes every pulse. One Scanner writes. Multiple Config Nodes read.',
  prerequisiteConcept: 'Scanner writes to Data Trail (A1-5). Config Node reads Data Trail (A1-3). Now they work together with multiple gates.',
  tapeDesignRationale: 'Eight-pulse tape [1,0,1,1,0,1,1,0] exercises both gate states multiple times. Longer tape gives more visual feedback for pattern recognition.',
  difficultyBand: 'derivable',
  narrativeFrame: 'The sensor grid reads the environment. Every sensor must get the current reading, not a stale one. Synchronization of information.',
  tutorialHints: [
...
export const levelA1_7: LevelDefinition = {
  ...
  budget: 40,
  scoringCategoriesVisible: ['efficiency', 'chainIntegrity', 'protocolPrecision'],
  computationalGoal: 'Build a complete three-layer machine: Scanner reads input to memory, Config Node gates on memory, Transmitter writes the gated result to output.',
  conceptTaught: 'The Transmitter completes the pipeline. It writes the result to the output tape. Scanner reads. Config Node gates. Transmitter records. Placement order is execution order.',
  prerequisiteConcept: 'Scanner-to-Config-Node pipeline (A1-5, A1-6). The player can build input-to-memory-to-gate chains. Now the chain needs an output endpoint.',
  tapeDesignRationale: 'Eight-pulse tape [1,1,0,1,0,0,1,1] with consecutive 0s (pulses 5-6) testing sustained blocking. Good distribution of both values.',
  difficultyBand: 'abstract',
  narrativeFrame: 'The weapons system needs authorization. Scanner reads the signal. Gate checks authorization. Transmitter sends the response. A complete decision chain.',
  tutorialHints: [
...
export const levelA1_8: LevelDefinition = {
  ...
  computationalGoal: 'Build a machine using all available piece types in a single coherent path that correctly processes a mixed input tape.',
  conceptTaught: 'Synthesis. Every concept from the sector applied in one machine. Routing, conditional gating, input reading, and output writing working together.',
  prerequisiteConcept: 'All Axiom concepts: routing (A1-1/A1-2), direction change (A1-2/A1-4), conditional gating (A1-3), Scanner pipeline (A1-5/A1-6), Transmitter output (A1-7).',
  tapeDesignRationale: 'Eight-pulse tape [1,0,1,1,0,1,0,1] with well-distributed values. Alternating pattern at end exercises rapid switching.',
  difficultyBand: 'abstract',
  narrativeFrame: 'The bridge is the last system. Everything the Engineer has learned comes together. The ship is either restored or it is not.',
  tutorialHints: [
