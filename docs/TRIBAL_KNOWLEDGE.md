...
- **Branch + PR model.** Every change reaches master through a branch (`feature/ fix/ refactor/ chore/ docs/`) and a pull request — agents open the PR with `gh pr create` (`gh` is installed and authenticated here: account TuckerBrady, repo+workflow scopes; plain `git` also works). CI runs the four gates on the PR (required checks: `Lint & Type Check`, `Unit & Integration Tests`, `Security Audit`). T-Bot reviews the diff on GitHub and approves, requests changes, or rejects; GitHub performs the merge. GitHub is the source of truth — the local repo is disposable (reclone to recover known-good). A trivial single-agent foreground commit may still go direct-to-master. Builds are cut only from master, only after the smoke gate. Authoritative model: T-Bot folder doc `REPO_GOVERNANCE.md`; in-repo detail in `project-docs/SPECS/branching-and-qa-strategy.md`.
...
- **Tone is load-bearing.** COGS dialogue (tutorial steps, cogsNotes, post-level lines, all of it) is team-authored and ships through T-Bot's PR voice review, no per-line sign-off (Tucker, 2026-09-21: "I trust the system now"). Canon, the True Ending dialogue, and non-COGS UI copy stay Tucker's. Full rule: COGS DIALOGUE DOCTRINE in `docs/NARRATIVE.md`.
